    ...
    group('when unpacking data, should', () {
      group('fail if', () {
        test('response is not a map', () {
          ...
          expect(() => driver.unpackData([1, 2, 3]), throwsA((String msg) => msg.contains('Expected result to be a map')));
        });
        test("response has no property 'data'", () {
          var driver = ListResultDriver();
          expect(() => driver.unpackData({'data': 'string'}), throwsA((String msg) => msg.contains("Expected 'data' key to be a List")));
        });
      });
    });
  });
